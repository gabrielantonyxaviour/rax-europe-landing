...
type PixelatedCanvasProps = {
  ...
  fadeSpeed?: number;
  /** 0..1 strength of dropout effect near pointer (dots disappear on hover). */
  hoverDropoutStrength?: number;
  /** Radius (px) around pointer for hover dropout effect. */
  hoverDropoutRadius?: number;
};
...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
  ...
  fadeOnLeave = true,
  fadeSpeed = 0.1,
  hoverDropoutStrength = 0,
  hoverDropoutRadius = 100,
}) => {
    ...
    img.onload = () => {
      ...
      const animate = () => {
        ...
        for (const s of samples) {
          ...
          // Hover dropout - skip drawing dots near pointer
          if (hoverDropoutStrength > 0 && activity > 0.01) {
            const dropoutFalloff = Math.max(0, 1 - dist / hoverDropoutRadius);
            const dropoutChance = dropoutFalloff * hoverDropoutStrength * activity;
            if (s.seed < dropoutChance) continue;
          }
          if (influence > 0.0005) {
